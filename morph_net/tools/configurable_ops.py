...
def _get_op_name(configurable_keras_layer_instance):
  ...
  if (is_strict and not matches_in_parameterization):
    raise KeyError(
        f'None of the following op names were found in the parameterization: {possible_op_names}'
    )

  if len(matches_in_parameterization) > 1:
    raise KeyError(
        f'Found multiple matching op names in the parameterization: {matches_in_parameterization}'
    )
  ...
  return f'{full_scope}/{default_op_suffix}'
...
def _get_fallback_rule_as_enum(fallback_rule):
  if not (isinstance(fallback_rule, (FallbackRule, str))):
...
class ConfigurableOps(object):
  ...
  def _mask(self, function, suffix, *args, **kwargs):
    ...
    *args[1].
    ...
    if op_scope := kwargs.get('scope') or kwargs.get('name'):
      if op_scope.endswith('/'):
        raise ValueError(
            f'Scope `{op_scope}` ends with `/` which leads to unexpected behavior.'
        )
      ...
      if function not in _OP_SCOPE_DEFAULTS:
        raise ValueError(
            f'No `scope` or `name` found in kwargs, and no default scope defined for {_get_function_name(function)}'
        )
      op_scope = _OP_SCOPE_DEFAULTS[function]
      full_scope = current_scope + op_scope
      if full_scope in self._scope_counts:
        new_scope = f'{full_scope}_{str(self._scope_counts[full_scope])}'
        self._scope_counts[full_scope] += 1
        full_scope = new_scope
      else:
        self._scope_counts[full_scope] = 1

    op_name = f'{full_scope}/{suffix}'
  ...
  def _get_function_and_suffix(self, key):
    ...
    if key not in self._function_dict:
      raise KeyError(f'Function "{key}" not supported by function_dict')
  ...
  def _pass_through_mask_list(self, fn_name, inputs_name, *args, **kwargs):
    ...
    if fn_name not in self._function_dict:
      raise ValueError(f'Unrecognized function name {fn_name}')
    fn = self._function_dict[fn_name]
    if args:
      inputs = args[0]
      args = args[1:]
    elif inputs_name in kwargs:
      inputs = kwargs.pop(inputs_name)

    else:
      raise ValueError(f'Missing `{inputs_name}` argument.')
    inputs = [t for t in inputs if not is_vanished(t)]
